...
namespace BooruBrowser.ViewModels
{
    public partial class SearchPageViewModel
    {
        partial void PartialConstructor()
        {
            ...
            QuerySubmittedCommand = new DelegateCommand<SearchBoxQuerySubmittedEventArgs>(ReSearch);
        }

        private void ReSearch(SearchBoxQuerySubmittedEventArgs args)
        {
            if (string.IsNullOrEmpty(args.QueryText))
                return;

            _originalSearchString = args.QueryText;
            _lastSearchString = args.QueryText;
            CurrentItemCollection = new IncrementalObservableCollection(CurrentDescriptor.Proxy, args.QueryText);
        }
        ...
        private void SuggestionClicked(SearchBoxResultSuggestionChosenEventArgs args)
        {
            ...
            if (image != null)
                App.Navigate(typeof(ImagePage), new Tuple<SiteDescriptor, IBooruImage>(CurrentDescriptor, image));
        }
        ...
        private async void SuggestionsRequested(SearchBoxSuggestionsRequestedEventArgs args)
        {
            ...
            if (!args.Request.IsCanceled)
            {
                ...
                {
                    args.Request.SearchSuggestionCollection.AppendQuerySuggestion(_originalSearchString);
                }
            }
